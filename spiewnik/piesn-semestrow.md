---
layout: song
title: "Pieśń semestrów"
song: true
section: towarzyskie
order: 209
permalink: /spiewnik-viribus-unitis-1883/piesn-semestrow/
sitemap: false
indexing: false
excerpt: "Pieśń semestrów — nuty i tekst ze Śpiewnika Weleckiego."
scores:
  - /assets/images/spiewnik/nuty/piesn-semestrow-1.png
lyrics: |
  A kto na pierwszym* semestrze jest
  Wstawaj, wstawaj, wstawaj,
  I biorąc kufel w ręce swe i pijąc i pijąc i pijąc do dna,
  Wstawaj, wstawaj, wstawaj,
  Pijany do domu wracaj.
  |: Wypił, wypił, nic nie zostawił, nic nie zostawił,
  Ażeby go, ażeby go Pan Bóg błogosławił. :|

  * w semestrze zimowym wstawiać kolejno: pierwszym, trzecim, piątym itd., a w semestrze letnim: drugim, czwartym, szóstym itd.
---
